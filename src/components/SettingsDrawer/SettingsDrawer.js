...
import { API_URL } from "../../model/helper";
...
const SettingsDrawer = ({ setIsSettingsOpen, isSettingsOpen }) => {
  ...
  const lastData = useMemo(() => {
    return populationData[
      Object.keys(populationData)[Object.keys(populationData).length - 1]
    ];
  }, [populationData]);
  ...
  const [addFormDefaultValue, setAddFormDefaultValue] =
    useState(addRowDefaultValue);
  ...
  const handleAddRowSubmit = () => {
    ...
    Object.keys(addFormDefaultValue).forEach((item) => {
      updatedData[item] = {
        ...updatedData[item],
        value:
          updatedData[item]?.type === "number"
            ? parseInt(addFormDefaultValue[item], 10)
            : addFormDefaultValue[item],
      };
    });
  ...
  return (
    ...
    >
      ...
      <Drawer.Body>
        <div>
          <InputGroup className="input-group-api-url">
            <ControlLabel className="form-labels">Api Url</ControlLabel>
            <Input readOnly value={API_URL} />
          </InputGroup>
        </div>
        <ol>
          {Object.keys(populationData)?.map((itemKey) => {
            const data = populationData[itemKey];
            return (
              <li key={itemKey}>
                <ConfigurationForm data={data} year={itemKey} />
              </li>
            );
          })}
        </ol>
        ...
        <Modal.Body>
          <Form
            ...
            onChange={(newFormValue) => {
              setAddFormDefaultValue({ ...newFormValue });
            }}
          >

...
const CollapsableMenu = (parentProps) => {
  ...
  return (
    <div className="collapsable-menu">
      ...
      <Collapse in={isOpen}>
        {(props) => <Panel {...props}>{children}</Panel>}
      </Collapse>
    </div>
  );
};
...
const ConfigurationForm = ({ data = {}, year = 2021 }) => {
  ...
  return (
    <Form
      ...
      onChange={(newData) => setFormValues({ ...newData })}
          ...
          return (
            <FormGroup key={eachKey + value}>
              <InputGroup inside className="input-group">
                <ControlLabel className="form-labels">{eachKey}</ControlLabel>
                <FormControl
                  name={eachKey}
